#!/usr/bin/env bash
# finalize.sh — Cluster 11 gaia-teach-testing skill finalize (E28-S89)
#
...
SCRIPT_NAME="gaia-teach-testing/finalize.sh"
WORKFLOW_NAME="teach-testing"
...
if [ -x "$CHECKPOINT" ]; then
  if ! "$CHECKPOINT" write --workflow "$WORKFLOW_NAME" --step 4 >/dev/null 2>&1; then
