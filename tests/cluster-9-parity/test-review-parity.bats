...
@test "E28-S70: SKILL.md frontmatter has tools containing Read" {
  head -20 "$SKILL_DIR/SKILL.md" | grep -q 'tools:.*Read'
}

@test "E28-S70: SKILL.md frontmatter has tools containing Grep" {
  head -20 "$SKILL_DIR/SKILL.md" | grep -q 'tools:.*Grep'
}

@test "E28-S70: SKILL.md frontmatter has tools containing Glob" {
  head -20 "$SKILL_DIR/SKILL.md" | grep -q 'tools:.*Glob'
}

@test "E28-S70: SKILL.md frontmatter has tools containing Bash" {
  head -20 "$SKILL_DIR/SKILL.md" | grep -q 'tools:.*Bash'
}

# ---------- AC1: Tool-allowlist enforcement (NFR-048) ----------

@test "E28-S70: SKILL.md tools does NOT contain Write" {
  local tools_line
  tools_line=$(head -20 "$SKILL_DIR/SKILL.md" | grep '^tools:')
  [[ "$tools_line" != *"Write"* ]]
}

@test "E28-S70: SKILL.md tools does NOT contain Edit" {
  local tools_line
  tools_line=$(head -20 "$SKILL_DIR/SKILL.md" | grep '^tools:')
  [[ "$tools_line" != *"Edit"* ]]
}
