...
#   2. Run validate-gate.sh for prereqs (artifact exists)
...
SCRIPT_NAME="gaia-val-validate/setup.sh"
...
# ---------- 2. Validate gate (artifact required) ----------
if [ -x "$VALIDATE_GATE" ]; then
  if ! "$VALIDATE_GATE" file_exists 2>&1; then
    die "HALT: Artifact not found — provide a valid artifact-path argument"
  fi
else
