...
# under E28-S35 (gaia-brainstorm/scripts/setup.sh). Adds create-ux-specific
# prereq gates:
#   - prd.md must exist in planning-artifacts (validate-gate file_exists)
#   - ux-design-assessment-template.md must exist in the skill directory or custom/templates/
#
# Responsibilities (per brief §Cluster 4):
#   1. Resolve config via the shared resolve-config.sh foundation script
#   2. Run validate-gate.sh for prereqs (prd.md, ux-design-assessment-template)
...
# create-ux requires a PRD to exist. The skill body validates the exact path;
# here we validate that the planning-artifacts directory exists at minimum.
...
# ---------- 2b. Guard: ux-design-assessment-template.md must be present ----------
# Check skill-local copy first, then custom/templates/ override. If neither
# exists, fail fast per AC2.
TEMPLATE_LOCAL="$SKILL_DIR/ux-design-assessment-template.md"
PROJECT_ROOT="${PROJECT_ROOT:-$(cd "$SKILL_DIR/../../../../.." && pwd)}"
TEMPLATE_CUSTOM="$PROJECT_ROOT/custom/templates/ux-design-assessment-template.md"

if [ ! -s "$TEMPLATE_LOCAL" ] && [ ! -s "$TEMPLATE_CUSTOM" ]; then
  die "ux-design-assessment-template.md missing — not found in skill directory ($TEMPLATE_LOCAL) or custom/templates/ ($TEMPLATE_CUSTOM). Cannot proceed."
fi
