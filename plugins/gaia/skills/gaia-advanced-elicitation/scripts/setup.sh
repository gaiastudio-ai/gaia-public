...
SCRIPT_NAME="gaia-advanced-elicitation/setup.sh"
WORKFLOW_NAME="advanced-elicitation"

# Resolve the GAIA plugin scripts directory from this script's location:
#   skills/gaia-advanced-elicitation/scripts/setup.sh → ../../../scripts
...
# advanced-elicitation's legacy workflow had no hard prereqs — it loads
# upstream research artifacts opportunistically in Step 1. Keep parity with
# the Cluster 4 reference — run the file_exists gate with zero --file
# arguments (no-op).
