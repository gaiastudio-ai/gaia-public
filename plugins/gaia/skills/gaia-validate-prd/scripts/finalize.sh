#!/usr/bin/env bash
# finalize.sh — Cluster 5 planning skill finalize (E28-S42, brief §Cluster 5 / P5-S3)
#
...
SCRIPT_NAME="gaia-validate-prd/finalize.sh"
WORKFLOW_NAME="validate-prd"
...
if [ -x "$CHECKPOINT" ]; then
  if ! "$CHECKPOINT" write --workflow "$WORKFLOW_NAME" --step 3 >/dev/null 2>&1; then
