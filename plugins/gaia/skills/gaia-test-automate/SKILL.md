...
Phase 2 executes the approved plan in main context with full write access. It performs five sub-steps with four HALT surfaces that prevent writes on any verification failure.

**Tool surface:** `[Read, Write, Edit, Bash, Grep, Glob]` -- same as `/gaia-dev-story`. Phase 2 is NOT a `context: fork` subagent (AC7, FR-TAF-1).

**Invocation:**
```bash
${CLAUDE_PLUGIN_ROOT}/skills/gaia-test-automate/scripts/phase2-execute.sh \
  --story "{story_key}" \
  --plan "docs/test-artifacts/test-automate-plan-{story_key}.md" \
  --ledger "{project_path}/.review-gate-ledger" \
  --test-env "docs/test-artifacts/test-environment.yaml" \
...
- `plan_tamper_detected` -- frontmatter internal divergence or parse failure (AC3)

**8.1 — SHA-256 drift detection (AC4):**
- For each entry in `analyzed_sources[]`, compute `shasum -a 256` and compare to stored hash
- Missing files are treated as mismatches
- HALT with `status: plan_drift` on any mismatch; emit list of diverged paths
- Empty `analyzed_sources[]` is a no-op (AC-EC1)

**8.2 — Target-path allowlist validation (AC5):**
- Load `test-environment.yaml` and derive tier-directory allowlist via `test-env-allowlist.sh`
- For each `proposed_tests[].test_file`: normalize path (resolve `..` and `.`), check prefix against allowlist
- Path-traversal attempts (`../`) are detected and rejected (AC-EC5)
- HALT with `status: target_path_out_of_scope` on any out-of-scope path

**8.3 — Test-file write (AC6, AC9):**
- For each `proposed_tests[]` entry: synthesize test-file content from `test_cases[]`
- Detect file extension to choose framework syntax (`.bats` -> bats, `.test.js` -> Jest/Vitest)
- Refuse to overwrite pre-existing files unless authored by a prior Phase 2 run (AC-EC10)
- Each file carries a `# Generated by phase2-execute.sh` header marker

**8.4 — Bridge execution (AC6, AC9, ADR-028):**
- Execute each written test file via the appropriate runner (bats for `.bats` files)
- Produce evidence file at `docs/test-artifacts/test-results/{story_key}-execution.json`
- Handle bridge unavailability gracefully: files remain on disk, evidence records `bridge_status: bridge_unavailable` (AC-EC8)

**8.5 — Record and update (AC8):**
- Update plan-file `phase` from `"approved"` to `"executed"` and append `execution_summary`
- Update story file Review Gate table: set "Test Automation" row status based on test results
- Inline update (not via `review-gate.sh` canonical glob) to support synthetic story files under test
