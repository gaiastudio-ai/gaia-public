#!/usr/bin/env bash
# setup.sh — gaia-val-validate-plan skill setup (E28-S77)
#
# Follows the shared setup.sh pattern from E28-S17.
# Validates that a plan artifact path argument was provided and the file exists.
#
# Responsibilities:
#   1. Resolve config via the shared resolve-config.sh foundation script
#   2. Run validate-gate.sh for prereqs (plan artifact exists)
...
SCRIPT_NAME="gaia-val-validate-plan/setup.sh"
WORKFLOW_NAME="gaia-val-validate-plan"
...
# ---------- 2. Validate gate (plan artifact required) ----------
if [ -x "$VALIDATE_GATE" ]; then
  if ! "$VALIDATE_GATE" file_exists 2>&1; then
    die "HALT: Plan artifact not found — provide a valid plan-artifact-path argument"
  fi
else
