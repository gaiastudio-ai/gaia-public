...
if [ "$ARTIFACT_REQUESTED" -eq 1 ] && [ ! -f "$ARTIFACT" ]; then
  ...
  printf 'Remediation: rerun /gaia-edit-arch to produce docs/planning-artifacts/architecture/architecture.md, then rerun finalize.sh.\n' >&2
  CHECKLIST_STATUS=1
  ...
  item_check "SV-01" "Output file saved to docs/planning-artifacts/architecture/architecture.md" \
